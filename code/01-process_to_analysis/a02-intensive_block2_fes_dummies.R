...
inf1 <- readxl::read_excel(paste0('../../../data/02-randomize/', country, 
  ...
  mutate(pais = country, batch_id = 'b1', 
         block2_fe = paste0(blockid2, country, batch_id))
...
foll <- foll |> left_join(inf |> select(author_id, block2_fe)) 

#write_parquet(foll |> select(-c(d, block1_fe)), 
#'../../../data/04-analysis/joint/BlocksIntensive/followers_filtered.parquet')

foll <- foll |> 
  select(follower_id, block2_fe)

# Convert to wide format
wide_f <- foll %>% mutate(value = 1) |>
  pivot_wider(
    names_from = block2_fe,
    values_from = value,
...
write_parquet(df1, '../../../data/04-analysis/joint/BlocksIntensive/original/intensive_fe_block2.parquet')
