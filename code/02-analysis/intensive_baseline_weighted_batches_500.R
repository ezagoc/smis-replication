...
stage <- "stage1_2"
list_types <- c("log_")
file_stub <- "intensive_baseline_weighted"
influencer_thr <- 9
n_posts_thr <- 0
n_permutations <- 500

results_root <- file.path("../../results", "intensive_baseline_weighted")
...
extract_weighted_treated <- function(data, outcome_vars, int_cols) {
...
write_outputs <- function(type, batch_name, original_coefs, permutation_coefs) {
  ...
  if (addon == "log ") {
    results_plot <- results_plot + xlim(-0.25, 0.2)
  }
