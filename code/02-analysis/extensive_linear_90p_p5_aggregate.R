...
list_types <- c("log_")
file_stub <- "extensive_verifiability_fes_normal_p90_p5_aggregate"
n_posts_thr <- 0
n_permutations <- 500

results_root <- file.path("../../results", "extensive_linear_90p_p5_aggregate")
...
for (type in list_types) {
  ...
  for (batch_name in names(batch_specs)) {
    ...
    original_coefs <- extract_total_treated(
      ...
      context_label = paste(batch_name, "aggregated original")
    )

    permutation_coefs <- map_dfr(seq_len(n_permutations), function(i) {
      message("Permutation ", i, " / ", n_permutations, " for ", batch_name, " - aggregated")
      ...
      extract_total_treated(
        data = permuted_df,
        outcome_vars = outcome_vars,
        context_label = paste(batch_name, "aggregated permutation", i),
        allow_missing = TRUE
      )
    })
