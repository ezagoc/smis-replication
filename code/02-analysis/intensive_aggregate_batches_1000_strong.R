...
sys.source("aggregate_batches_intensive.R", envir = aggregate_env)
aggregate_data <- aggregate_env$aggregate_data

results_root <- file.path("../../results", "intensive_aggregate_batches_strong")
...
file_stub <- "log_intensive_aggregate_batches_strong"
n_permutations <- 1000
...
for (batch_name in names(batch_specs)) {
  message("Running aggregated intensive strong sample: ", batch_name)
  ...
  original_coefs <- extract_weighted_treated(
    ...
    context_label = paste(batch_name, "aggregate intensive strong original")
  )

  permutation_coefs <- map_dfr(seq_len(n_permutations), function(i) {
    message("Permutation ", i, " / ", n_permutations, " for ", batch_name, " - aggregate intensive strong")
    ...
    extract_weighted_treated(
      ...
      context_label = paste(batch_name, "aggregate intensive strong permutation", i),
      allow_missing = TRUE
    )
  })
