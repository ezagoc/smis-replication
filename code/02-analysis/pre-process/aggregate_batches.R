...
# These variables are summed across treatment stages.
...
# These baseline variables are kept from stage 1-2 and merged back after aggregation.
baseline_ver_vars <- c(
  "total_shares_base",
  "total_comments_base",
  "total_likes_base",
...
# These are the sample-defining columns we want to carry through unchanged.
...
baseline_keys <- bind_rows(
  read_parquet(file.path(analysis_root, "KE", "baseline", "baseline_divided_b1b2.parquet")) |>
    mutate(pais = "KE"),
  read_parquet(file.path(analysis_root, "SA", "baseline", "baseline_divided_b1b2.parquet")) |>
    mutate(pais = "SA")
) |>
  select(follower_id, username, pais, batch_id) |>
  distinct()

attrition <- read_parquet(file.path(analysis_root, "attrition.parquet")) |>
  mutate(batch_id = "b2")
...
# A long version is useful for checks before collapsing across stages.
