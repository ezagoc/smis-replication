...
results_root <- results_path("intensive_linear_95p_weighted_strong")
...
collect_stage_data <- function(stage, batch_filter = NULL) {
  ...
  all_endline_vars <- setdiff(
    names(stage_df),
    c(id_cols, "total_treated", "total_influencers", "below_p90", "c_t_strong_total", "weights", all_base_vars)
  )
