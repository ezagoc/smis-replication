...
outcome_map <- c(
  ver = "log_verifiability_base",
  non_ver = "log_non_ver_base",
  true = "log_true_base",
  fake = "log_fake_base",
  n_posts = "log_n_posts_base",
  eng = "log_eng_base",
  n_posts_covid = "log_n_posts_covid_base",
  pos_b_covid = "log_pos_b_covid_base",
  neutral_b_covid = "log_neutral_b_covid_base",
  neg_b_covid = "log_neg_b_covid_base",
  n_posts_vax = "log_n_posts_vax_base",
  pos_b_vax = "log_pos_b_vax_base",
  neutral_b_vax = "log_neutral_b_vax_base",
  neg_b_vax = "log_neg_b_vax_base"
)
...
extract_total_treated <- function(data, outcomes, context_label = "", allow_missing = FALSE) {
  estimates <- map_dbl(unname(outcomes), function(outcome_var) {
    fmla <- as.formula(
      paste0(
        outcome_var, " ~ total_treated + dummy_second + dummy_third | ",
        "pais + batch_id"
      )
    )
...
eng_df <- get_analysis_english_winsor(
  ...
  select(follower_id, pais, batch_id, log_eng_base)
...
sent_df <- get_analysis_sent_bert_final2(
  ...
  select(
    ...
    log_n_posts_covid_base,
    log_pos_b_covid_base,
    log_neutral_b_covid_base,
    log_neg_b_covid_base,
    log_n_posts_vax_base,
    log_pos_b_vax_base,
    log_neutral_b_vax_base,
    log_neg_b_vax_base
  )
