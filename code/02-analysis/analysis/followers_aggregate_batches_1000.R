...
extract_aggregate_followers <- function(data, int_cols, context_label = "", allow_missing = FALSE) {
...
analysis_df <- load_followers_aggregate_data(initial_path) |>
  filter(below_p90 == 1) |>
...
for (batch_name in names(batch_specs)) {
  message("Running followers aggregate sample: ", batch_name)
  ...
  original_coefs <- extract_aggregate_followers(
    data = interaction_fit$data,
    int_cols = interaction_fit$int_cols,
    context_label = paste(batch_name, "aggregate original")
  )

  permutation_coefs <- map_dfr(seq_len(n_permutations), function(i) {
    message("Permutation ", i, " / ", n_permutations, " for ", batch_name, " - aggregate")
    ...
    extract_aggregate_followers(
      data = permuted_interactions$data,
      int_cols = permuted_interactions$int_cols,
      context_label = paste(batch_name, "aggregate permutation", i),
      allow_missing = TRUE
    )
  })
