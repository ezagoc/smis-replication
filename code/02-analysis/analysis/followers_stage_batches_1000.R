...
extract_stage_followers <- function(data, int_cols, context_label = "", allow_missing = FALSE) {
  ...
  stats::setNames(as.list(estimates), c("AC", "SMIs")) |>
    as_tibble()
}
...
write_outputs <- function(batch_name, stage, original_coefs, permutation_coefs) {
  file_code <- paste0(file_stub, "_", batch_name, "_", stage, "_1000perm")
...
for (batch_name in names(batch_specs)) {
  message("Running followers stage sample: ", batch_name)
  ...
  for (stage in list_stages) {
    ...
    base_df <- load_followers_stage_data(stage, initial_path) |>
      filter(below_p90 == 1) |>
    ...
    interaction_fit <- build_intensive_interactions(base_df)

    original_coefs <- extract_stage_followers(
  ...
  write_xlsx(
    ...
    file.path(estimates_dir, paste0(file_stub, "_", batch_name, "_1000perm_estimates.xlsx"))
  )

  write_stage_plot(
    final = final,
    plot_path = file.path(plots_dir, paste0(file_stub, "_", batch_name, "_1000perm.pdf")),
    ylab_text = "Total treated estimate with 95% confidence interval"
  )
}
