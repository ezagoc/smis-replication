...
sys.source("../pre-process/aggregate_batches.R", envir = aggregate_env)
aggregate_data <- aggregate_env$aggregate_data

# 3.0 Define constants and output locations
results_root <- file.path("../../../results", "extensive_aggregate_batches")
...
file_stub <- "log_extensive_aggregate_batches"
n_permutations <- 1000
...
for (batch_name in names(batch_specs)) {
  message("Running aggregated sample: ", batch_name)
